...
int loadAccounts(struct Account account[], int size)
{
    ...
    if (fp != NULL) {
        while (fscanf(fp, "%d~%c~%s~%d~%lf~%s\n", &account[i].accountNum, &account[i].accountType, account[i].Person.fullName, &account[i].Person.yearOfBirth,
            &account[i].Person.TotIncome, account[i].Person.residenceCountry) != EOF) 
        {
            counter += 1;
            i ++;
        }
...
int loadTickets(struct Ticket ticket[], int size)
{
    ...
    if (fp != NULL) {
        while (fscanf(fp, "%d|%d|%d|%s|%d|", &ticket[i].ticketNumber, &ticket[i].accountNumber, &ticket[i].activity, ticket[i].subject,
            &ticket[i].countMessages) != EOF)
        {
            for ( j = 0; j < ticket[i].countMessages; j++)
            {
                fscanf(fp, "%c|%s|%s|", &ticket[i].message[j].accountType, ticket[i].message[j].fullName, ticket[i].message[j].message);
            }
            counter += 1;
            i ++;
        }
